...
class Model extends Model_Nonbdd
{
	...
	function get_relations($requester,$notfollow=array(),$blockfollow=false,$getRelFields=true)
	{
		if(!$blockfollow)
		{
			foreach ($this->relations as $rel)
			{
				...
				if (!in_array($rel['name'],$notfollow))
				{
					...
					$model->set_uid($this->uid.'_'.$rel['uid']);
					...
					if ($rel['follow'] === true)
					{
						$requester = $model->get_relations($requester,$rel['notfollow'],$rel['blockfollow'],$rel['champs']!=='*aaa');
					}
				}
			}
		}
	...
	function save($values=null)
	{
		...
		if (!empty($this->values[$this->primaryKey]))
		{
            if(!$this->allowUpdate)
            {
                \HTTP::error_page('500','It\'s not allow to update this entity'); //@TODO Message en francais comme partout ailleurs
            }
		...
		else
		{
            if(!$this->allowInsert)
            {
                \HTTP::error_page('500','It\'s not allow to insert this entity'); //@TODO Message en francais comme partout ailleurs
            }
	...
	function delete($arg=null,$csrf=0)
	{
	    if(!$this->allowDelete)
        {
            \HTTP::error_page('500','It\'s not allow to delete this entity'); //@TODO Message en francais comme partout ailleurs
        }
