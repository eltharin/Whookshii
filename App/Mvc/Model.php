...
class Model extends Model_Nonbdd
{
    ...
    function save($values=null,$params=[])
    {
		...
		if($this->forecReplaceInto == true)
		{
            ...
            {
                \HTTP::error_page('500','Action incorrecte'); //@TODO Message en francais comme partout ailleurs
            }
		...
		elseif (!empty($this->values[$this->primaryKey]))
		{
            if(!$this->allowUpdate)
            {
                \HTTP::error_page('500','Vous ne pouvez pas mettre à jour cette entrée');
            }
            if (!$this->validates(false))
            {
                \form::set_old_values(array_merge($this->vars,$this->values));
        ...
        else
        {
            if(!$this->allowInsert)
            {
                \HTTP::error_page('500','Vous ne pouvez pas ajouter cette entrée');
            }
    ...
    function delete($arg=null,$csrf=0)
    {
        if(!$this->allowDelete)
        {
            \HTTP::error_page('500','Vous ne pouvez pas supprimer cette entrée');
        }
